...
class VenueDBBase(SQLModel):
    ...
    def cast_data_object(self, data_object_class) -> Venue:
...
class VenueDBCreate(VenueDBBase):
    @model_validator(mode='before')
    def validate_fields(cls, fields):
...
class VenueDBRead(VenueDBBase):
    pass


class VenueDB(VenueDBBase, table=True):
...
class VenueFilter(BaseModel):
    ...
    # def apply_filters(self, database_object_class: VenueDBBase, query: select) -> select:
